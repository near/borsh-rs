...
pub struct FindTyParams<'ast> {
    ...
    associated_type_params: HashSet<Ident>,
...
impl<'ast> FindTyParams<'ast> {
    pub fn new(generics: &Generics) -> Self {
        ...
        FindTyParams {
            ...
            associated_type_params: HashSet::new(),
            associated_type_usage: Vec::new(),
        }
    }
    pub fn process_for_bounds(self) -> Vec<TypePath> {
    ...
    pub fn process_for_params(self) -> Vec<Ident> {
        let relevant_type_params = self.relevant_type_params;
        let associated_type_params = self.associated_type_params;

        let mut params: Vec<Ident> = relevant_type_params
            .into_iter()
            .chain(associated_type_params.into_iter())
            .collect();
        params.sort_by_key(|type_path| type_path.to_token_stream().to_string());
        params
    }
}
impl<'ast> FindTyParams<'ast> {
        ...
        if let Type::Path(ty) = ungroup(&field.ty) {
            if let Some(Pair::Punctuated(t, _)) = ty.path.segments.pairs().next() {
                if self.all_type_params.contains(&t.ident) {
                    ...
                    self.associated_type_params.insert(t.ident.clone());
                }
            }
        }
