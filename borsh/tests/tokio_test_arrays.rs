...
macro_rules! test_array {
    ($v: expr, $t: ty, $len: expr) => {
        let buf = $v.try_to_vec().await.unwrap();
        let actual_v: [$t; $len] = AsyncBorshDeserialize::try_from_slice(&buf)
            .await
...
#[tokio::test]
async fn test_custom_struct_array() {
    let arr = [CustomStruct(0), CustomStruct(1), CustomStruct(2)];
    let serialized = arr.try_to_vec().await.unwrap();
    let deserialized: [CustomStruct; 3] = AsyncBorshDeserialize::try_from_slice(&serialized)
        .await
        .unwrap();
...
async fn test_string_array() {
    ...
    let serialized = arr.try_to_vec().await.unwrap();
    let deserialized: [String; 3] = AsyncBorshDeserialize::try_from_slice(&serialized)
        .await
        .unwrap();
    assert_eq!(arr, deserialized);
}
